...
if (
    isset($_REQUEST['sce']) && ( strtolower($_REQUEST['sce']) == 'view' or
...
// get the metar data from the cache or from the URL if the cache is 'stale'
        ...
        $age = abs(time() - $metarGMT);
// age in seconds
        $Debug .= "<!-- age=$age sec '$metarDate' -->\n";
        mtr_load_iconDefs();
// initialize ICON defs to use and lookup arrays
...
// remove internal newlines
        $metar = preg_replace('|/////KT|is', 'VRB00KT', $metar);
// replace bogus wind report
        $metar = preg_replace('|@|is', '', $metar);
// remove strange @ in metar
        $metar = preg_replace('|///|is', ' ', $metar);
// remove strange standalone slashes
        $metar = preg_replace('| /|is', ' ', $metar);
// remove strange standalone slashes
        $metar = preg_replace('| / |is', ' ', $metar);
// remove strange standalone slashes
        $metar = preg_replace('| \s+|is', ' ', $metar);
// remove multiple spaces
        $metar = preg_replace('| COR |i', ' ', $metar);
// remove COR (correction) from raw metar
        $metar = preg_replace('|(\d{5}) KT|i', '${1}KT', $metar);
// fix any space in wind value
        $metar = preg_replace('| 999 |', ' 9999 ', $metar);
// fix malformed unlimited visibility
        $metar = preg_replace('| LRA |', ' -RA ', $metar);
// fix malformed light rain
        $metar = preg_replace('| HRA |', ' +RA ', $metar);
// fix malformed light rain
      ...
      // $metar = preg_replace('| (\d+) (\d+)/(\d+)SM |i',' $1_$2/${3}SM ',$metar); // fix NOAA visibility
        ...
        mtr_process($metar, $icao);
// actually parse the metar for conditions.. results in $mtrInfo array
...
function mtr_gzdecode($rawdata, &$error = '', &$filename = '', $maxlength = null)
{
    ...
    if (strcmp(substr($data, 0, 2), "\x0d\x0a")) {
// got an extra \r\n in the data?
...
// CURRENT CONDITIONS ICONS FOR clientraw.txt
   ...
   // to the value provided in clientraw.txt
    $Icons[0] = "day_clear.gif";
// image sunny.visible
    $Icons[1] = "night_clear.gif";
// image clearnight.visible
    $Icons[2] = "day_partly_cloudy.gif";
// image cloudy.visible
    $Icons[3] = "day_partly_cloudy.gif";
// image cloudy2.visible
    $Icons[4] = "night_partly_cloudy.gif";
// image night cloudy.visible
    $Icons[5] = "day_clear.gif";
// image dry.visible
    $Icons[6] = "fog.gif";
// image fog.visible
    $Icons[7] = "haze-sm.gif";
// image haze.visible
    $Icons[8] = "day_heavy_rain.gif";
// image heavyrain.visible
    $Icons[9] = "day_mostly_sunny.gif";
// image mainlyfine.visible
    $Icons[10] = "mist-sm.gif";
// image mist.visible
    $Icons[11] = "fog.gif";
// image night fog.visible
    $Icons[12] = "night_heavy_rain.gif";
// image night heavyrain.visible
    $Icons[13] = "night_cloudy.gif";
// image night overcast.visible
    $Icons[14] = "night_rain.gif";
// image night rain.visible
    $Icons[15] = "night_light_rain.gif";
// image night showers.visible
    $Icons[16] = "night_snow.gif";
// image night snow.visible
    $Icons[17] = "night_tstorm.gif";
// image night thunder.visible
    $Icons[18] = "day_cloudy.gif";
// image overcast.visible
    $Icons[19] = "day_partly_cloudy.gif";
// image partlycloudy.visible
    $Icons[20] = "day_rain.gif";
// image rain.visible
    $Icons[21] = "day_rain.gif";
// image rain2.visible
    $Icons[22] = "day_light_rain.gif";
// image showers2.visible
    $Icons[23] = "sleet.gif";
// image sleet.visible
    $Icons[24] = "sleet.gif";
// image sleetshowers.visible
    $Icons[25] = "day_snow.gif";
// image snow.visible
    $Icons[26] = "day_snow.gif";
// image snowmelt.visible
    $Icons[27] = "day_snow.gif";
// image snowshowers2.visible
    $Icons[28] = "day_clear.gif";
// image sunny.visible
    $Icons[29] = "day_tstorm.gif";
// image thundershowers.visible
    $Icons[30] = "day_tstorm.gif";
// image thundershowers2.visible
    $Icons[31] = "day_tstorm.gif";
// image thunderstorms.visible
    $Icons[32] = "tornado.gif";
// image tornado.visible
    $Icons[33] = "windy-sm.gif";
// image windy.visible
    $Icons[34] = "day_partly_cloudy.gif";
// stopped rainning
    $Icons[35] = "windyrain-sm.gif";
// wind + rain
...
';
    ...
    // load the parser condition strings
          $recin = trim($rec);
        if ($recin and substr($recin, 0, 1) <> '#') {
        // got a non comment record
...
function mtr_fixupTime($intime)
{
    ...
    $t2 = join(':', $t);
// put time back to gether;
    $t2 = preg_replace('/[^\d\:]/is', '', $t2);
// strip out the am/pm if any
...
//   This function directs the examination of each group of the METAR. The problem
    ...
    $lang = 'en';
    foreach ($mtrInfo as $i => $value) {
    // clear out prior contents
         unset($mtrInfo[$i]);
    //    $mtrInfo[$i] = '';
    }
//  $Debug .= "<!-- wxInfo cleared\n" . print_r($mtrInfo,true) . " -->\n";
    ...
    if ($metar != '') {
        ...
        $groupName = array('mtr_get_station',
                       ...
                       'mtr_get_altimeter');
        $metarPtr = 3;
// mtr_get_station identity is ignored
        $group = 1;
// start with Time
        ...
        while ($group < count($groupName)) {
            ...
            $groupName[$group]($part);
// $groupName is a function variable
        }
    } else {
...
function mtr_get_wind($part)
{
    ...
    if (preg_match('/^([0-9G]{5,10}|VRB[0-9G]{2,7})(KT|MPS|KMH)$/', $part, $pieces)) {
        ...
        // no wind
            ...
            if ($unit == 'KT') {
                $speed = 1.1508 * $pieces[2];
    // from knots
            } elseif ($unit == 'MPS') {
                $speed = 2.23694 * $pieces[2];
            // from meters per second
            } else {
                $speed = 0.621371 * $pieces[2];
            // from km per hour
            }
...
function mtr_get_visibility($part)
{
    ...
    // good weather
        if (preg_match('|mph|i', $UOMS['WIND'])) {
            $mtrInfo['VISIBILITY'] = 'greater than 7 miles (10 km)';
// or 10 km
        } else {
            $mtrInfo['VISIBILITY'] = 'greater than 10 km';
        // or 10 km
        }
...
// Decodes current weather conditions. This function maybe called several times
    ...
    if (preg_match('|^R\d\d|', $part)) {
// more than one runway conditions report?
        $metarPtr++;
    }
    static $conditions = '';
    ...
    if (preg_match('/^(-|\+|VC)?(TS|SH|FZ|BL|DR|MI|BC|PR|RA|DZ|SN|SG|GR|GS|PE|IC|UP|BR|FG|FU|VA|DU|SA|HZ|PY|PO|SQ|FC|SS|DS)+$/', $part, $pieces)) {
        ...
        $join = (strlen($conditions) == 0) ? '' : ', ';
    // append conditions with a ', ' between
        if (substr($part, 0, 1) == '-') {
            $prefix = 'Light ';
            $part = substr($part, 1);
            ...
            $prefix = '';
        }  // moderate conditions have no descriptor
    ...
    // remove trailing comma if any
        $conditions = preg_replace('| , |is', ', ', $conditions);
    // replace space before comma
        $conditions = preg_replace('|\s+|is', ' ', $conditions);
    // remove multiple spaces
...
function mtr_get_altimeter($part)
{
    ...
    if (preg_match('/^(A|Q)([0-9]{4})/', $part, $pieces)) {
        ...
        // units are inches Hg, converts to hectoPascals
            ...
            $pressureHPA = (int) $pieces[2];
        // units are hectoPascals
            $pressureIN = round(0.02953 * $pressureHPA, 2);
        // convert to inches Hg
        }
