...
if (isset($SITE['uomBaro'])) {
    $uomBaro = $SITE['uomBaro'];
}
if (isset($SITE['uomWind'])) {
    $uomWind = $SITE['uomWind'];
}
if (isset($SITE['uomRain'])) {
    $uomRain = $SITE['uomRain'];
}
if (isset($SITE['fcsticonstype'])) {
...
// now $mtr array contains the decoded values
    foreach ($toDisplay as $key => $legend) {
        if (!isset($mtr[$key]) or strlen($mtr[$key]) < 1) {
            continue;
        }
        ...
        if ($key == 'WIND') {
            ...
            $dirlabel = $t[0];
            if (isset($t[0])) {
                $t[0] = langtransstr($t[0]); // translate wind direction
            }
            if (isset($t[1])) {
                $t[1] = ''; // remove 'at'
            }
    ...
    <td align="center" valign="top" class="data1" style="text-align: center;border: none">
      <?php if (isset($mtr['WIND'])) { ?>
                  ...
                  <?php $wr = $condIconDir . $wrName . $dirlabel . $wrType; // normal wind rose
                    ...
                    if (
                        preg_match('!(calm|varies)!i', $mtr['WIND']) and
                             ($wrCalm <> '')
                    ) { // use calm instead
