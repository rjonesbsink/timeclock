...
### New Install
 ...
 - Give this user at least SELECT, UPDATE, INSERT, DELETE, ALTER, and CREATE privileges to ONLY 
    this database.
 -  Open setup.php with your web browser. It creates the database tables (using create_tables.sql),
    an initial admin account with a password you choose, and optionally the weather sidebar --
    config.inc.php no longer needs to be edited by hand for these. Restrict access to setup.php
    (IP allowlist, HTTP auth, or delete it) once setup is complete; it refuses to run again while
    config.inc.php exists.
 -  Open index.php with your web browser.
 -  Click on the Administration link on the right side of the page and log in with the admin
    username and password you chose during setup.
